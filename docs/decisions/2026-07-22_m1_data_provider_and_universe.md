...
|------|-----|
| 文档类型 | decision |
| 状态 | proposed（rev.2，已回应评审） |
| 日期 | 2026-07-22 |
| 策略版本 | SMM-V1.0.0（本 ADR 不改变策略规则，故不 bump；但 §2 新增 config 键会改变 `config_hash`，见 §2） |
...
| 变更摘要 | 冻结 M1 四项决策：yfinance 默认源（optional extra）、静态带日期股票池快照、Bar 双价格表示（adj_factor 派生）、fixture 改为确定性生成器 |
...
**版本影响：** 新增该键会改变 `config_hash`，但 **不 bump `SMM-V1.0.0`**。理由：它是数据治理护栏，不是策略规则（不影响任何一条过滤、评分或仓位逻辑）。**此判断请 reviewer 确认**——如认为凡进入 YAML 者皆须版本纪律，我按你的界线改。
...
### 3. `Bar` 双价格表示：未复权为主字段，复权经 `adj_factor` 派生
...
| 成交、止损、跳空判定（MVP-B） | `open`/`high`/`low`/`close` | `adj_close` / `adj_factor` |
...
**字段方向的理由：** 未复权价是**当时真实发生的事实**，复权价是随后续公司行动而变化的派生量。把事实放主字段、派生量放附加字段，可以让误用在代码里更显眼——任何拿 `bar.close` 去算动量的地方都是可被 review 抓到的错误，反之则不然。这也与 §1.1 的 PIT 局限一致：会随下载日漂移的那个量，不应该是主字段。
...
7. §3.3 的消费边界必须以类型/视图强制，并有单测；不接受"靠 code review 保证"。
