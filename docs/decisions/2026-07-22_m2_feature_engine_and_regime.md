...
## 排名宇宙、缺失语义与数值定义（rev.2，回应 PR #6 评审）
...
| 排除 | 理由 |
|------|------|
| benchmark 符号（SPY、11 只板块 ETF） | 它们是**基准**不是候选。让 XLK 与个股比 RS 是拿口径不同的东西排名 |
| `insufficient_history` 标的 | 特征算不出来，不能占据百分位 |
| §12.4 校验失败标的 | 数据不可信 |

**分量各自独立**：`Rank(RS_Sector_63)` 只对当日该分量非 missing 的标的做百分位；被排除的标的整份 `RelativeStrengthScore` 已为 missing（§2），不进这张表。动量 `Rank_21/63/126` 同理，只对收益可算的标的排名。

排名以**硬过滤前**的合格池为基准（宪法 §17.2「当前股票池」），不是过滤后的幸存者——否则排名会随过滤强度漂移。

### R2. SPY 缺失 = 整次 run 失败

**明确选定：SPY 数据缺失或校验失败时，整次 features/regime run 以非零码退出**，不产出半份产物。

**不采纳**「输出 `regime=missing` 的部分产物」：SPY 缺失时 `RS_SPY_63`/`RS_SPY_126` 对**每一只**标的都缺失，`RelativeStrengthScore` 全员 missing，候选池必然为空。产出一份没有任何候选、却看起来正常的日报，只会制造误读风险。

**绝对禁止：** 把 missing 显示成 Risk-On，或在 regime 未知时按 Neutral 继续。

### R3. 数值定义（冻结，禁止实现时随手选）

宪法未钉死的部分在此冻结。跨实现漂移会让回放结果不可复现。

| 量 | 定义 |
|----|------|
| 百分位排名 | **average ties**，输出域 **0–100**（宪法 §17.3） |
| EMA20 | 以前 20 根的 SMA 作种子，其后递归 `α = 2/21`。**不**用首根收盘作种子——那会让前几十根带上初值偏差 |
| ATR20 | **Wilder 平滑**（ATR 的原始定义），非 TR 的简单均值。需额外 warm-up，已计入 `min_history_bars` |
| SMA 斜率 | `(SMA_t / SMA_{t-N}) - 1`，N 进 config（默认 20）。用**相对变化**而非绝对差值，否则高价股斜率恒大于低价股，横截面不可比 |
| 63 日回撤 | `min(close_t / cummax(close)[t] - 1)`，窗口内最小值 |
| 延伸度 | `(close - EMA20) / ATR20` |

### R4. 美元成交额用 `adj_close × volume`

保持「特征层只读 `AdjustedBar`」的单一入口不破例。

**误差可忽略：** 20 日平均只用最近 20 根 bar，而后向复权的 `adj_factor` 在当下附近收敛于 1.0，偏差远低于 1%——对 $20M 的流动性门槛没有实质影响。为一个不影响判定的小数点破坏 M1 §3.3 已由类型强制的边界，不划算。

### R5. 范围澄清

| 项 | 归属 |
|----|------|
| `relative_volume` | **M3**（属触发器，Plan v1.1 §4.5），M2 不计算 |
| QQQ 确认、市场宽度 | **非 M2 范围**（宪法 §14 列为可选），实现 PR 不得擅自加进硬条件 |
| 板块 ETF 的 ingest | M2 实现 PR 需显式拉取 11 只 ETF 并过 §12.4；**不得**写入股票池快照 |
