...
|------|-----|
| 文档类型 | decision |
| 状态 | accepted（rev.2；[PR #18](https://github.com/xxxxxthhh/swingMomentum/pull/18) 二次评审接受） |
| 日期 | 2026-07-22 |
...
6. 当日触发在下一交易日仍是非终态 `TRIGGERED`；若日报只有「新触发 / 观察池 / 终态变化」，它会在尚未被 Risk/Paper 消费前从观察窗口消失。
...
`source=synthetic` 与 `source=market` 只决定 provider/universe，不改变后续 scanner/report 代码路径。

market 默认路径可经 `DataProvider` ensure/fetch/cache 所需 bars，以满足 Plan 的单入口编排；业务代码不得在 provider 外导入或调用 yfinance。calendar/session/validation 任一不完整均 fail closed；「忘记先 ingest」不得降级成正常的空报告。实现可提供诊断性 `--cache-only`，但 cache 不足时只能失败，且不改变默认路径。`source=synthetic` 与 N 日复现门禁必须完全离线，不得访问网络。
...
```text
data/runs/<strategy_version>/<config_hash>/
```

实现必须 sanitize 两个路径段。一旦 root 中存在任何 batch seal 或完成 manifest，后续日任务必须使用相同的 `strategy_version` 与 `config_hash`。不同 identity 必须换新 root；M4 不提供原地迁移、覆盖开关或人工拼接历史。`data/cache` 可跨 run 共享，但 run/artifact root 不得混 identity。

这条在「当前没有 active signal」时仍适用。不能因为没有可重放实体，就把两个 config 的日期悄悄拼成同一观察窗口。

provider source、universe snapshot identity/hash 与 git commit 写入 manifest 供审计，但不进入 `signal_id` / `setup_key`。git SHA 可获得时记录真实 SHA；脱离 git 安装时只能写 literal `unknown`，不得伪造 hash，也不得因此拒绝运行。

### 4. 日报同时表达当前状态与当日事件，并保留未处理触发

Markdown 固定四节，CSV 用互斥 `bucket` 表达相同内容；每个 signal 在一份 CSV 中只出现一次：

| bucket / Markdown 节 | 定义 |
|----------------------|------|
| `new_trigger` / 新触发 | 当日 transition 的 `to_state == TRIGGERED`，含 `DETECTED → TRIGGERED` 与 `WATCHLISTED → TRIGGERED` |
| `open_trigger` / 未处理触发 | 当前状态为 `TRIGGERED`，且当日**没有**进入 `TRIGGERED` 的 transition；用于保留尚未被 MVP-B Risk/Paper 消费的跨日 backlog |
...
同日 `DETECTED|WATCHLISTED → TRIGGERED` **只**归 `new_trigger`，不得同时进入 `open_trigger`。`new_trigger` 也优先于通用「状态变化」，避免同一 transition 重复两行。历史终态不在后续每日重复展示。

展示 bucket 顺序固定为：

```text
new_trigger → open_trigger → watchlist → terminal_change
```
...
- bucket 内排序仅为 `MomentumScore` 降序、`RelativeStrengthScore` 降序、symbol 升序，missing 排末。排序不改变 signal 状态或资格。
...
现有 config schema 的 `execution.execution_mode: paper_trading` 是更后阶段的冻结配置，**不构成** M4 生成 order/fill/position 或启用 Paper 的授权。M4 只认 manifest 的 `mvp_a_signal` 边界；实现不得把该 YAML 默认值解释为可执行路径。
...
5. 第二个全新 root 重放同一序列，CSV/Markdown/manifest（除明确排除的运行时字段外）逐字节一致；两个 root 位于同一 tree SHA 时，`git_commit` 必须包含在逐字节比较内；
6. 同 root 精确重跑不改 bytes；跳日、旧日回填、同日修订、config 漂移全部 fail closed。

manifest 不记录 wall-clock、随机 run id 或绝对临时路径。若两个环境合法地处于不同 tree SHA，只能在 manifest 的复现契约中显式把 `git_commit` 列为排除字段；不得静默忽略。否则「逐字节可复现」会被无业务意义或未声明字段破坏。
...
| 只列当日 `new_trigger`，不列跨日 `open_trigger` | 非终态触发在 MVP-B 消费前从观察窗口消失，无法审计 standing backlog |
...
1. 回归先行：先证明 event-only 报告会漏掉 silent watchlist，且三桶报告会漏掉 carried `TRIGGERED`，再扩 scanner/report seam。
...
| 项 | 决策 |
|----|------|
| market fetch vs cache-only | 默认经 `DataProvider` ensure/fetch/cache；可选诊断性 cache-only 只能 fail closed；synthetic/复现门禁全离线（见 §1） |
| 默认 artifact root | 必须按 `data/runs/<strategy_version>/<config_hash>/` 隔离；cache 可共享，run root 不混 identity（见 §3） |
| git commit 缺失语义 | 可得则真实 SHA，不可得只能 `unknown`；仅审计且复现排除必须显式（见 §3、§8） |
...
| 2026-07-22 | proposed (rev.2) | 回应 PR #18 评审：增加跨日 `open_trigger`；将 market/provider、identity root、git commit 三项裁决写入正文；钉死现有 `paper_trading` 配置不构成 M4 执行授权 |
| 2026-07-22 | accepted (rev.2) | 二次评审接受（B1、O1–O3 全部落入正文）；两条 residual 属实现约束，转入 M4 实现 PR 的验收项，不改本 ADR 正文 |
