...
#### 2.1.1 Shadow external snapshot canonical artifact basis

Issue #60 Task Reviewer comment `5061866645` freezes the first runtime-input
seam without enabling M7 orchestration: shadow receives an explicit external
`PortfolioSnapshot` with exact X identity, validates it fail closed, and
normalizes the parsed model into a named per-session canonical artifact before
any future manifest. The artifact, rather than its transport path, is the
replay fact; a future shadow manifest must bind its SHA-256 alongside the
other named artifacts. Account values remain operational facts, not config.

The operator input need not already be byte-canonical. After strict parsing,
the canonical artifact uses deterministic JSON with sorted keys and stable
set ordering. Same-day idempotency compares those normalized artifact bytes:
key order or whitespace-only source-file changes are a no-op, while a parsed
value change at the same `(as_of, strategy_version, config_hash)` is a
fail-closed conflict. The source path, raw file bytes, wall-clock time, and
random identifiers do not enter the artifact, hash, or config identity.

This first slice is shadow-only preflight/input persistence. It neither changes
the default M4 mode nor calls Risk Engine, writes transitions or ledgers, or
enables paper. Paper continuation still requires a separately accepted
complete ledger-to-`PortfolioSnapshot` projector; current partial ledgers
cannot be used to infer a portfolio fact.
...
实现反推的默认值，Slice 1 对每个通过 X-identity 校验的外部 `PortfolioSnapshot` 固定
输入：

```text
realized_loss_r_for_session = 0
marked_equity               = snapshot.account_equity
prior_high_water_equity     = snapshot.account_equity
integrity_halt              = false
```

因此产出的 `CircuitState` 固定记录 `high_water_equity == marked_equity`、
`drawdown == 0`、无 circuit reason，且正常 entry multiplier 为 1。这里的
`prior_high_water_equity` 相等**不是**对持续账户实际没有既往回撤的推断；它只是
Stage A 的 bounded neutral shadow 语义：本路径没有可验证的既往 high-water 或当日
realized-loss 事实，因而不会假装已计算过 drawdown-reduce / drawdown-stop。

这项限制必须同时由 runtime integration regression 读取 canonical
`circuit_state.json` 并钉死其输入/identity，防止未来改动静默改变该语义。它不授权
变更 `PortfolioSnapshot`、Paper、ledger、live broker 或 config；也不能通过 current
equity、open symbols、exposure 或 risk map 反推缺失的事实。

当 Paper 事实来源可用时，必须在独立的已接受 ADR 中定义 prior high-water、session
realized-loss、X identity/provenance 和缺失事实的 fail-closed 规则，随后才可取代这个
bounded neutral 输入。该工作由 [Issue #69](https://github.com/xxxxxthhh/swingMomentum/issues/69)
跟踪，且不属于 PR #68 或 Slice 2 的 N-day reproducibility gate。
