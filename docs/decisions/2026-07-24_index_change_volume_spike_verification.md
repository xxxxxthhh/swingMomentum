...
| 变更摘要 | 25 倍门槛继续作为异常检测器；仅当版本化的 S&P 官方成分变更事实精确解释生效前一交易日放量时，原始成交量可不经修改地通过；缺失或冲突仍整轮 fail-closed |
...
| 标的 | 官方记录中的精确 ticker |
| 验证 session | effective date 前最后一个 provider session |
...
6. 事件 effective date 必须是 provider calendar 中 `S` 的下一 session；
...
2. policy、允许的 index/action/source catalog 与 session offset 进入
...
| effective date 不是下一 provider session | fail-closed |
