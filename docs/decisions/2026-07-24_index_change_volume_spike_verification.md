...
| 讨论依据 | [Issue #77](https://github.com/xxxxxthhh/swingMomentum/issues/77)；Task Reviewer comment `5067778024`；[Issue #85](https://github.com/xxxxxthhh/swingMomentum/issues/85) FER 真实运行阻塞与用户批准 |
| 变更摘要 | 25 倍门槛继续作为异常检测器；仅当版本化的 S&P 500 或 Nasdaq-100 官方成分变更事实精确解释生效前一交易日或生效日放量时，原始成交量可不经修改地通过；index 与官方来源 host 必须成对匹配，缺失或冲突仍整轮 fail-closed |
...
### 2. v2 确定性规则覆盖股票池中的两个官方指数来源

初版只覆盖 S&P 500；FER 的真实运行证明股票池中的 Nasdaq-100-only 标的也会因
同类官方重构产生合法极端成交量。用户批准后，v2 只增加 Nasdaq-100 官方成分
变更，并保留相同的 point-in-time、T-1/T 与 fail-closed 约束：

| 维度 | 冻结范围 |
|------|----------|
| 权威来源 | S&P 500 → S&P Global / S&P DJI 官方公告；Nasdaq-100 → Nasdaq 官方公告 |
...
S&P 400/600、其它指数、ETF rebalance、财报、并购、增发或其它新闻事件仍不在
规则内。未来扩展必须新增 accepted ADR 或明确修订本 ADR 的 source catalog、
时序和测试，不得把“官方指数事件”泛化为任意叙事解释。
...
4. 事件 index 为 `S&P 500` 或 `Nasdaq-100`，action 为 `addition` 或
   `deletion`，且 source host 必须属于该 index 的冻结官方 host 列表；
...
执行路径不得在 daily run 中临时搜索网页或抓取新闻。旧 S&P-only 快照继续可
重放；v2 起使用签入仓库的通用带日期事件快照，例如：

```text
configs/market_events/YYYY-MM-DD_index_constituent_changes.csv
```
...
| FER 2025-12-19 + Nasdaq 官方 addition 记录 | 原始 `62,023,100` 不变地通过，并写 verification |
| Nasdaq-100 事件使用 S&P host（或反向） | fail-closed |
...
- 规则范围仍只对应正式股票池的两个指数，可用 CASY 与 FER 真实样本做 hermetic
  回归。
...
- 当前规则只解决 S&P 500 与 Nasdaq-100 成分变更，未来其它指数/事件仍可能停机；
...
| 2026-07-24 | accepted | PR #78 合并首个 S&P 500 确定性验证规则 |
| 2026-07-24 | accepted (rev.2) | Issue #85 真实 FER 阻塞后，用户明确批准 Nasdaq-100 官方成分变更扩展；最终实现仍须按精确 HEAD 接受 Reviewer 复审 |
