...
2. **既有缓存不会重验。** `covers` 为真时直接读缓存,不跑完整性检查。本次改动之后写入的数据受检,**之前写入的陈旧缓存可能带洞继续服务**。普通供应商异常先原命令重跑:
...
S&P 500 官方 addition/deletion 记录，且 spike 恰好发生在 effective date
前一 provider session 或 effective date 当天，原始 volume 才可继续进入计算。

遇到停机时：

1. 不提高 25 倍阈值、不剔除 symbol、不改写 Parquet；
2. 用 S&P Global / S&P DJI 一手公告核实 ticker、index、action、公告日与生效日；
3. 新增一份日期更晚的 CSV 快照，保留旧快照，不覆盖历史；
...
第三方新闻、搜索摘要、其它指数事件或运行时网页抓取不能让异常通过。缺失、
重复、冲突、未来公告或窗口不匹配均维持整轮 fail-closed。
